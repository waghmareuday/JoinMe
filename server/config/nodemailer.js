...
// 🟢 This function dynamically generates a fresh access token and creates the transporter
const createTransporter = async () => {
  try {
    ...
    return nodemailer.createTransport({
      ...
      auth: {
        type: 'OAuth2',
        user: process.env.SMTP_USER, // MUST be the exact Gmail you used for Google Cloud!
        accessToken,
...
export const sendOTPEmailFunc = async (to, otp) => {
  ...
  try {
    ...
    console.error('❌ EXACT OAUTH ERROR:', error.message);
    return false;
  }
};
